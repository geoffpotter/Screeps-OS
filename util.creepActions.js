 ...
 * var mod = require('util.creepActions');
 * mod.thing == 'a thing'; // true
 */

var logger = require("screeps.logger");
logger = new logger("util.creepActions");
...
class creepActions {
    ...
    pickupEnergy(creep, range=false, notNearController=false, pileId=false) {
        if (pileId) {
            ...
            var target = creep.pos.findClosestByRange(FIND_DROPPED_RESOURCES, {filter:function(r){
                return r.resourceType == RESOURCE_ENERGY && r.amount > creep.carryCapacity*2;
            }});
            if (!target) {
                target = creep.pos.findClosestByRange(FIND_DROPPED_RESOURCES, {filter:function(r){
...
module.exports = new creepActions();