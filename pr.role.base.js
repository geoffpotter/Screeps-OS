...
class baseRole extends processClass {
    ...
    updateFillTasks() {
        ...
        for(let c in this.creeps) {
            ...
            if (this.allowRefils) {
                ...
                if (!deliverTask) {
                    deliverTask = this.taskManager.createTask(this, global.Task.DELIVERENERGY, global.Task.TYPE_WORK, creep.pos, {"targetId":creep.id});
            ...
            if (!creep.spawning) {
                ...
                if (task) {
                    ...
                    if (task) {
                        ...
                        creep.memory.task = {name:task.name, pos:task.pos};
                    } else {
    ...
    runCreepTasks() {
        for(let creepName in this.creepTasks) {
            ...
            let creep = _.find(this.creeps, {'name':creepName});
            if (!creep) {
                //guess he died?
