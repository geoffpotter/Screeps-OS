...
buildingManager.prototype.makeBuildPlan = function() {
  ...
  for (var i in sources) {
    var s = sources[i];
    var _this = this;
    var p = PathFinder.search(baseLoc, {
      pos: s.pos,
      range: 1
    }, {
      plainCost: 2,
  ...
  //plan storage
  var pathToStorage = PathFinder.search(baseLoc, {
    pos: this.room.controller.pos,
    range: 3
  }, {
    plainCost: 2,
  ...
  if (mineral) {
    mineral = mineral[0];
    this.buildPlan.extractor = {
      x: mineral.pos.x,
      y: mineral.pos.y
    };

  }
  var pathToExtractor = PathFinder.search(storagePos, {
    pos: mineral.pos,
    range: 1
  }, {
    plainCost: 2,
...
buildingManager.prototype.addExtentionsToPath = function(path, assumeDiag) {
  var lastPos = (new RoomPosition(path[0].x, path[0].y, path[0].roomName));
  if (!lastPos) {
    return false;
  }
  if (assumeDiag) {
  ...
  for (var i in path) {
    var pos = path[i];
    var pos2 = (new RoomPosition(pos.x, pos.y, pos.roomName));
    var pos3 = (new RoomPosition(pos.x, pos.y, pos.roomName));
    pos2.x += 1;
    pos3.x += 2;
    // if (i%2==0) {
    //         pos2.y += 1
    ...
    for (var j = 0; j < 4; j++) {
      ...
      if (onDiag) {
        if (this.canPlace(pos2)) {
          this.buildPlan.extensions.push({
            x: pos2.x,
            y: pos2.y
          });
        }
...
buildingManager.prototype.buildStructures = function(type, locations, numNeeded) {
  var exts = this.room.find(FIND_STRUCTURES, {
    filter: (s) => {
      return s.structureType == type

    }
  });
  var constructions = this.room.find(FIND_CONSTRUCTION_SITES, {
    filter: (s) => {
      return s.structureType == type
