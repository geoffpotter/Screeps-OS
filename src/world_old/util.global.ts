...
let logger = new logger_imported("util.global");
    ...
    for (var roomName in flow) {
      ...
      for (var x in flow[roomName]) {
        for (var y in flow[roomName][x]) {
          ...
          visual.text(charMap[dir], pos.x, pos.y + 0.3, {
            color: 'red',
            font: 0.8
          });
        }
...
global.utils = utilStatic;

var stats_imported = import ./screeps.stats;
let stats = new stats_imported();

var traveler_import = require('./util.traveler');
var traveler_setup = traveler_import({
  exportTraveler: true,
...
let traveler = new traveler_setup();

var creepRequest = import ./util.creepRequest;

var market = import ./util.market;
...
global.utils = utilFull;
export default utilFull;