...
let logger = new logger_imported("util.market");
...
export default {
  ...
  watchMarket: function() {
    ...
    for (var o in orders) {
      ...
      if (!buySellPrices[order.resourceType])
        buySellPrices[order.resourceType] = {
          buy: [],
          sell: []
        };

      buySellPrices[order.resourceType][order.type].push({
        price: order.price,
    ...
    for (var type in buySellPrices) {
      ...
      var maxBuyPrice = _.max(_.map(prices.buy, function(p) {
        return p.price
      }));
      var minSellPrice = _.min(_.map(prices.sell, function(p) {
        return p.price
      }));
  ...
  sellMyMinerals: function(roomName, terminalContents) {
    if (!Memory.marketPrices || !terminalContents) {
      return;
    }
    //terminalContents = (new RoomPosition(terminalContents.x, terminalContents.y, terminalContents.roomName));
