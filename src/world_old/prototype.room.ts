...
let logger = new logger_imported("prototype.room");
...
Room.prototype.getNonFullStorage = function(noSpawn) {
  ...
  if (!noSpawn) {
    var spawnConts = this.find(FIND_STRUCTURES, {
      filter: function(s) {
        return s.structureType == STRUCTURE_CONTAINER && s.getSpawn() && _.sum(s.store) < s.storeCapacity
      }
    });
  ...
  if (targets.length == 0) {
    ...
    var conts = this.find(FIND_STRUCTURES, {
      filter: function(s) {
        return s.structureType == STRUCTURE_CONTAINER && !s.getSpawn() && !s.getMineral()
      }
    });
    if (conts.length > 0) {
...
Room.prototype.getStuffForPickup = function(includeAll) {
  ...
  var spawnConts = this.find(FIND_STRUCTURES, {
    filter: function(s) {
      return s.structureType == STRUCTURE_CONTAINER && s.getSpawn() && s.mineralAmount() > 0
    }
  });
  if (spawnConts.length > 0) {
    ...
    var conts = this.find(FIND_STRUCTURES, {
      filter: function(s) {
        return s.structureType == STRUCTURE_CONTAINER && (includeAll || (!s.getSpawn() || s.getSpawn() && s.mineralAmount() > 0))
      }
    });
    if (conts.length > 0) {
      targets = targets.concat(conts);
    ...
    var piles = this.find(FIND_DROPPED_RESOURCES, {
      filter: function(p) {
        return p.amount > 100
      }
    });
