...
var logger = new logger_imported("manager.empire");
...
var empireManager = function() {
  this.roomManagers = {};
}
var base = require('./manager.base');
empireManager.prototype = new base();


var utils = require('./util.global')

utils.extendFunction(empireManager, "init", function() {
...
utils.extendFunction(empireManager, "tickInit", function() {
...
utils.extendFunction(empireManager, "tick", function() {
  let roomsStart = Game.cpu.getUsed();
...
utils.extendFunction(empireManager, "tickEnd", function() {
  for (var roomName in this.roomManagers) {
    var rm = this.roomManagers[roomName];
    rm.tickEnd();
  }
...
empireManager.prototype.getHomeRooms = function() {
  var homeRooms = [];
...
empireManager.prototype.findJobForCreep = function(currentRoom, creep, baseRole) {
  var options = this.roomManagers;
  var _this = this;
  var optionsWithDistance = _.map(options, function(r) {
...
empireManager.prototype.getHomeRoomByRange = function(fromLoc) {
  ...
  optionsWithDistance = _.filter(optionsWithDistance, function(r) {
    return r.dist < 3
  })
...
empireManager.prototype.getRemoteEnergyRoomsByRange = function(startRoom, range) {
  ...
  rooms = _.sortBy(rooms, function(r) {
    return r.energyInRoom + r.mineralsInRoom
  });
  //logger.log("////",startRoom, _.map(rooms, (c) => c.roomName))
  return rooms;

}
empireManager.prototype.createOwnedRooms = function() {
...
empireManager.prototype.createRemoteMiningRooms = function() {
...
empireManager.prototype.createClaimRooms = function() {
  for (var i in Game.flags) {
    var flag = Game.flags[i];
    var roomName = flag.pos.roomName;
...
empireManager.prototype.createMurderRooms = function() {
...
empireManager.prototype.createKeeperRooms = function() {
...
empireManager.prototype.requestHelperCreep = function(roomManagerNeedingHelp, creepNeeded) {
  ...
  helpers = _.sortBy(helpers, function(rm) {
    return rm.room.energyAvailable
  });
...
empireManager.prototype.higestRCL = function() {
  var highest = 0;
...
empireManager.prototype.killMe = function(roomName) {
  this.roomManagersToDelete.push(roomName);
}
empireManager.prototype.killDeadRooms = function() {
