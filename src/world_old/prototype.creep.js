...
let logger = new logger_imported("prototype.creep");
...
Creep.prototype.flee = function(range) {
  var nearCreeps = this.pos.findInRange(FIND_HOSTILE_CREEPS, range, {
    filter: i => i.isHostile() && (i.getActiveBodyparts(ATTACK) > 0 || i.getActiveBodyparts(RANGED_ATTACK) > 0)
  });
  var nearKeeperLairs = this.pos.findInRange(FIND_HOSTILE_STRUCTURES, range, {
    filter: function(s) {
  ...
  if (nearCreeps.length > 0) {
    var ret = PathFinder.search(this.pos, _.map(nearCreeps, i => ({
      pos: i.pos,
      range: range
    })), {
      maxRooms: 1,
