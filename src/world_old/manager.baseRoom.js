...
let logger = new logger_imported("manager.baseRoom");
//logger.enabled = false;
var obj = function() {}

var roles = require('./role.internal');
...
obj.prototype.getRoleStatus = function() {
  ...
  if (this.remoteEnergyRooms) {
    var s = _.map(this.remoteEnergyRooms, function(r) {
      return r.roomName
    });
...
obj.prototype.assignCreeps = function() {
  ...
  var ourCreeps = _.filter(creepNames, function(c) {
    return Memory.creeps[c].home == _this.roomName
  });
  this.numCreeps = 0;
  for (var c in ourCreeps) {
    ...
    if (!creep) {
      ...
      var roleObj = _.find(this.roleObjects, function(r) {
        return r.name == memory.role
      });
