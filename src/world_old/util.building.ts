...
let logger = new logger_imported("util.building");
...
export default {
  ...
  canPlace: function(buildPlan, posObj) {
    logger.log("canPlace", posObj, JSON.stringify(posObj))
  ...
  addExtentionsToPath: function(buildPlan, path, assumeDiag) {
    ...
    for (var i in path) {
      ...
      if (pos2.x < 49) pos2.x += 1;
      if (pos3.x < 48) pos3.x += 2;
      console.log('here??')
      // if (i%2==0) {
      //         pos2.y += 1
      //     }
      ...
      for (var j = 0; j < 4; j++) {
        ...
        if (onDiag) {
          if (this.canPlace(this.buildPlan, pos2)) {
            buildPlan.extensions.push({
              x: pos2.x,
              y: pos2.y,
              roomName: pos2.roomName
            });
          }
  ...
  rotate: function(center, pos, angle) {
    ...
    pos.x = Math.max(Math.min(Math.floor(nx), 49), 1);
    pos.y = Math.max(Math.min(Math.ceil(ny), 49), 1);
