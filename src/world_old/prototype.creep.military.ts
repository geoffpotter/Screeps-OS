...
let logger = new logger_imported("prototype.creep.military");
...
Creep.prototype.healCreepsNearMe = function(useRanged, useHeal) {
  ...
  var injuredInRange = this.pos.findInRange(FIND_MY_CREEPS, 3, {
    filter: function(c) {
...
Creep.prototype.healCreeps = function() {
  var friends = this.room.find(FIND_MY_CREEPS, {
    filter: function(c) {
      //logger.log(c.name, c.hits , c.hitsMax)
      return c.hits < c.hitsMax;
    }
