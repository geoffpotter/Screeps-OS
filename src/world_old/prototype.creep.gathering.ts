...
let logger = new logger_imported("prototype.creep.g");
...
Creep.prototype.getStuffFromSourceContainers = function(noSmallAmounts) {
  ...
  var creep = this;
  var conts = this.room.find(FIND_STRUCTURES, {
    filter: function(s) {
      return s.structureType == STRUCTURE_CONTAINER && (s.getSource() || s.getMineral()) && !s.getSpawn() && _.sum(s.store) > creep.carryCapacity * 0.5
    }
  });
  var closestCont = this.pos.findClosestByRange(conts);
  if (!closestCont && !noSmallAmounts) {
    conts = this.room.find(FIND_STRUCTURES, {
      filter: function(s) {
        return s.structureType == STRUCTURE_CONTAINER && (s.getSource() || s.getMineral()) && !s.getSpawn() && _.sum(s.store) > 200
      }
    });
...
Creep.prototype.getMineralsFromContainers = function() {
  var conts = this.room.find(FIND_STRUCTURES, {
    filter: function(s) {
      return s.structureType == STRUCTURE_CONTAINER && (_.sum(s.store) - (s.store.energy > 0 ? s.store.energy : 0)) > 200
    }
  });
  if (conts.length) {
