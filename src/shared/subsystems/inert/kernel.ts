...
import INeRTQueues from './queue/types';
import Stat from 'shared/utils/stat';
import Logger from 'shared/utils/logger';
import profiler from 'shared/utils/profiling/profiler';
const logger = new Logger('INeRT.kernel');
logger.color = 'green';

interface KernelMemory {
  procMem: { [key: string]: any };
}

// Remove or update these global declarations as needed
declare global {
    interface Memory {
        INeRT?: KernelMemory;
    }
...
export default class Kernel {
  public procTable: { [key: string]: Process } = {};
  public queues: INeRTQueues;
  public cpuUsed: Stat;
  public overhead: Stat;
  public memOverhead: Stat;
  public threadsRun: Stat;
  public memory: KernelMemory;
  public lastTick: number | false = false;
  public time: number = 0;
  public cpuLimit: number = 1000;
  public cpuDefcon: number = 0;
  public cpuBucketRecoverTicks: number = 0;
  public profile: boolean = true;
  private cpuAtStart: number = 0;
  private threadCpu: number = 0;
  private totalCpuUsed: number = 0;
  private wrappedProcs?: { [key: string]: number };

  constructor() {
    this.queues = new INeRTQueues();
  ...
  startProcess(process: Process, parentProc?: Process): void {
    if (!(process instanceof Process)) {
      throw new Error("invalid process:" + process);
    }
    if (parentProc && !(parentProc instanceof Process)) {
      throw new Error("invalid process:" + parentProc);
    }
    ...
    process.parentProc = parentProc ?? undefined;
    ...
    if (this.profile) {
      ...
      if (!this.wrappedProcs[process.constructor.name]) {
        ...
        profiler.registerClass(process, process.constructor.name);
  ...
  destroyProcess(process: Process): void {
    ...
    //destroy child procs
    let childProcs = _.filter(this.procTable, (p) => p.parentProc == process);
  ...
  getProcess(name: string): Process | false {
  ...
  run(): void {
    ...
    let cpuByProc: { [key: string]: number } = {};

    //run threads
    /** @type {threadClass} */
    let thread: Thread | false = false;
    ...
    while(thread = this.queues.getNextThread()) {
      ...
      if (ret === Thread.DONE || ret === Thread.HUNGRY) {
        if (ret === Thread.DONE) {
  ...
  manageProcArray<T>(
    arrayOfStuff: T[],
    processClass: new (name: string, data: any) => Process,
    procNameFN: (thing: T) => string,
    procDataFN: (thing: T) => any
  ): void {
