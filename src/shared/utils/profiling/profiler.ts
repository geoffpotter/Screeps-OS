...
function outputProfilerData(): string {
  ...
  output += padHeader("Ignore/Call", 12);
  output += padHeader("CPU/Tick", 12);
  output += padHeader("Ignore/Tick", 12);
  output += padHeader("Tot CPU", 12);
  output += padHeader("Tot Calls", 12);
  output += padHeader("% of Tot\n", 12);

  ////  Data lines
  data.forEach((d) => {
    output += padHeader(`${d.name}`, longestName, false);
    output += padHeader(`${d.callsPerTick.toFixed(2)}`, 12);
    output += padHeader(`${d.cpuPerCall.toFixed(2)}ms`, 12);
    output += padHeader(`${d.cpuPausedPerCall.toFixed(2)}`, 12);
    output += padHeader(`${d.cpuPerTick.toFixed(2)}ms`, 12);
    output += padHeader(`${d.cpuPausedPerTick.toFixed(2)}`, 12);
    output += padHeader(`${d.totalCpu.toFixed(2)}ms`, 12);
    output += padHeader(`${d.calls}`, 12);
    output += padHeader(`${(d.cpuPerTick / totalCpu * 100).toFixed(0)} %\n`, 12);
  });
