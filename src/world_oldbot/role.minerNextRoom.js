...
let logger = new logger_imported("role.minerNextRoom");
...
utils.extendFunction(obj, "init", function(name, homeRoomManager, sourceLoc, homeOverride) {
  ...
  if (this.minerCreep) {
    ...
    if (!this.dangerZone || !this.minerCreep.flee(5)) {
      if (!this.minerCreep.pos.isNearTo(new RoomPosition(targetPos.x, targetPos.y, targetPos.roomName))) {
        ...
        global.utils.moveCreep(this.minerCreep, new RoomPosition(targetPos.x, targetPos.y, targetPos.roomName), opts);
