...
nodeNetwork.maxEdgeUpdatesPerTick = 100;
...
export const loop = () => {
    ...
    if (spawn) {
        ...
        for (const role of Object.values(ROLES)) {
            if (Object.keys(creepCounts).length !== Object.keys(ROLES).length && creepCounts[role] && creepCounts[role] > 0) {
                continue;
            }
...
function manageUpgrader(creep: Creep) {
    if (creep.store.getUsedCapacity() == 0) {
        ...
        if (creep.room.controller) {
            moveCreep(creep, creep.room.controller.pos, 3);
    ...
    if (creep.store.getFreeCapacity() > 0) {
        ...
        if (target) {
            ...
            if (controller && creep.pos.getRangeTo(controller) > 3) {
                moveCreep(creep, controller.pos, 3);
                moved = 3;
            } else {
