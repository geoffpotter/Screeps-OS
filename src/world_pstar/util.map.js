...
var cm = require("util.cm");
var visual = require("util.visual");
var WorldPosition = require("util.worldposition");
...
function dirStrToPath(startNode, dirStr) {
    if (dirStr.length == 0) {
        return [];
    }
...
class CachedPath {
    ...
    getPath() {
        ...
        if (!this.path && this._cachedPath) {
            this.path = dirStrToPath(this.orgin, this._cachedPath);
        }
        if (this.path) {
            return this.path;
        }
        ...
        opts.roomCallback = (roomName) => {
            ...
            let cm = cm.getCM(roomName, "pStar");
            return cm;
        }
    ...
    serialize() {
        ...
        //logger.log(this.orgin, JSON.stringify(map.dirStrToPath(this.orgin, this._cachedPath)));
    ...
    static deserialize(str) {
        ...
        if (cachedPath && cachedPath != 0 && cachedPath != "false") {
            ...
            inst.path = dirStrToPath(orgin, cachedPath);
        }
