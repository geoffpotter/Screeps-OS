...
class pStarProc extends processClass {
    ...
    init() {
        ...
        if (Memory.pStarCache) {
            ...
            pstar.inst = pstar.classes.pStar.deserialize(Memory.pStarCache);
            let used = Game.cpu.getUsed() - start;
            logger.log("pStar deserialize.  CPU:", used, "Size:", Memory.pStarCache.length);
            //logger.log("wtf",JSON.stringify(pstar.inst));
        }
    }
    initThreads() {
        return [
    ...
    pStarSave() {
        //serialize
        let start = Game.cpu.getUsed();
        Memory.pStarCache = pstar.inst.serialize();
        let used = Game.cpu.getUsed() - start;
    ...
    getNodeById(nodeId) {
        return pstar.inst.getNode(nodeId);
    }
    hasNode(node) {
        return pstar.inst.hasNode(node);
    }
