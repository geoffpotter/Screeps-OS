...
class serializeFlag {
    ...
    loadFlag() {
        ...
        visual.circle(this.pos, "green", 1, 0.5);
    }
    displayFlag() {
        if (this.pos) {
            //we have pos, display flag name at pos
            visual.drawText(this.flagName, this.pos);
        } else {
    ...
    static deserialize(str) {
        ...
        if (pos) { //if pos is there, the others should be too
            inst.pos = WorldPosition.deserialize(pos).toRoomPosition();
            inst.color = color;
...
class testProc extends processClass {
    init() {
        ...
        this.flags = new array.classes.IndexingCollection("id", ["pos.roomName", "color"], [3, 5, 8]);
    }
    initThreads() {
        return [
    ...
    init_onTick() {
        if (Memory.flags) {
            //this.flags = array.IndexingCollection.deserialize(Memory.flags, serializeFlag);
        }
    ...
    save() {
        ...
        //let deseri = array.IndexingCollection.deserialize(Memory.flags, serializeFlag)
