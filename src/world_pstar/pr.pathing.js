...
class pathingProc extends processClass {
    ...
    updateNetwork() {
        ...
        if (numRefined > 0 || (numEdgesRefined > 0 && this.totalEdgesRefined < 10)) { // if we did work, there's prolly more work to do!
            ...
            pstar.inst.addRoomsToNetwork(1);//add a max of one room at a time.
    ...
    init_onTick() {
        this.totalEdgesRefined = 0;
        //pstar.inst = new pstar.classes.pStar();

        try {
            this.mapFlags(true);

                // pstar.inst.addRoomsToNetwork();//dumb
                // pstar.inst.refineRooms();//dumber
                // pstar.inst.addRoomsToNetwork();//dumb
                // pstar.inst.refineRooms();//dumber
                // pstar.inst.addRoomsToNetwork();//dumb
                // pstar.inst.refineRooms();//dumber
        ...
        // pstar.inst.addRoomToAdditionQueue("E11S1");



        //Game.cpu.halt();
        // if (Memory.edges) {
        //     this.edges = global.utils.array.IndexingCollection.deserialize(Memory.edges, pstar.Edge)
        // }
        //return threadClass.DONE;
    }
    save() {
    ...
    run() {
        ...
        if (Memory.pStar) {
            //pstar.inst = {};
            //pstar.inst = pstar.class.deserialize(Memory.pStar);
        }




        //pstar.inst.refineEdges();

        //

        // start = Game.cpu.getUsed();
        // let deserialized = pstar.class.deserialize(serialized);
        ...
        //try {
            ...
            //pStar a*
            // start = Game.cpu.getUsed();
            // let pStarPath = pstar.findPath(src, dest, 1000, refinePaths);
            // used = Game.cpu.getUsed() - start;
            // logger.log(src.id, dest.id, "cpu used", used, JSON.stringify(pStarPath))
            // pstar.inst.displayFindPath(pStarPath.path, "#f00");

            // //this.displayNetwork();

            // // //pStar a*
            // // start = Game.cpu.getUsed();
            // // let pStarPath2 = pstar.findPath(src2, dest, 1000, refinePaths);
            // // used = Game.cpu.getUsed() - start;
            // // logger.log(src2.id, dest.id, "cpu used2", used, JSON.stringify(pStarPath2))
            // // pstar.inst.displayFindPath(pStarPath2.path, "#00f");
    ...
    mapFlags(forceAll = false) {
        ...
        if (!pstar.inst.hasNode(sNode)) {
            logger.log("adding base flag", sNode.id);
            pstar.inst.addNode(sNode);
        }
        ...
        for(let i in otherFlags) {
            ...
            // let edge = new pstar.Edge(sNode, node);
            // if (!this.edges.has(edge)) {
            //     this.edges.add(edge);
            // }
            /** @type {pStarRoom} */
            let room = pstar.inst.getRoom(node.pos.roomName);
            if (!room) {
                room = new pStarRoom(node.pos.roomName);
                pstar.inst.rooms.add(room);
                room.refineRoom();
            }
