...
let intel = require("util.intel");
let roomIntel = intel.classes.RoomIntel;
let roomStatus = intel.classes.RoomStatus;
let roomObjectIntel = intel.classes.RoomObjectIntel;

let action = require("util.action");
let actionTypes = action.classes.actionTypes;
...
class roomManager extends processClass {
    ...
    manageController() {
        if (!this.praiseAction) {
            ...
            this.praiseAction = action.createAction(actionTypes.PRAISE, this.intel.controller.id, this.intel.controller.pos, amts);
    ...
    display() {
        ...
        visual.drawTextLines(lines, new RoomPosition(1, 1, this.roomName));
    }
}
