...
class roomObject extends processClass {
    ...
    getAction(actionType, initFN = (a) => {}) {
        ...
        if (!this.actions[actionType]) {
            this.actions[actionType] = action.createAction(actionType, this.data.targetId, this.data.pos, {});
        ...
        for(let c in creeps) {
            ...
            if (creep.spawning) {
                ...
                if (creep.pos.inRangeTo(dest, 2)) {
                    ...
                    pStar.inst.moveTo(creep, {pos: dest, range:1});
                }
            }
        }
    ...
    displaySpawnQueue(queue) {
        ...
        let spawn = this.getObject();
        visual.drawTextLines(lines, spawn.pos)
    }
