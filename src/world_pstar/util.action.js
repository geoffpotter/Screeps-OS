    ...
    constructor(actionType, range, priority, filterFN = false, sortFN = false) {
        ...
        if (!sortFN) {
            this.sortFN = (creep, action) => {
                let range = pstar.findDistance(creep.pos, action.pos);
                return range;
            }
        } else {
            this.sortFN = sortFN;
...
class baseAction {
    ...
    amountRemainingByPriorityAndPos(priority, pos) {
        ...
        let targetRange = pstar.findDistance(pos, this.pos);
        ...
        this.assignments.forEach((assignment) => {
            ...
            if (assignedObj) {
                ...
                range = pstar.findDistance(this.pos, assignedObj.pos);
    ...
    display() {
        visual.drawText(this.actionType + "(" + Object.keys(this.assignments.getAll()).length + ")", this.pos);
    }
        ...
        if (this.assignments.getAll().length >= this.maxAssignments) {
            ...
            if (priority == lowestPriority) {
                ...
                let allAssignments = _.sortBy(this.assignments.getAll(), (a) => {
                    ...
                    return pstar.findDistance(this.pos, assignmentObj.pos)
                }).reverse();
                ...
                if (farthestAssignedObj) {
                    ...
                    logger.log(creep, "stealin jobs", this.id, pstar.findDistance(this.pos, farthestAssignedObj.pos), pstar.findDistance(this.pos, creep.pos))
                    if (
                        pstar.findDistance(this.pos, farthestAssignedObj.pos)
                            > pstar.findDistance(this.pos, creep.pos)
                        ) {
    ...
    moveToPosition(creep) {
        if (!this.pos.inRangeTo(creep, this.targetRange)) {
            pstar.inst.moveTo(creep, {pos:this.pos, range: this.targetRange});
        }
