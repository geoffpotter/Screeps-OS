...
let logger = new logger_imported("role.transporter");
...
obj.prototype.runCreep = function(creep) {
  ...
  if (creep.memory.transporting == true && !creep.memory.targetContainer) {
    ...
    if (targetCont.length > 0) {
      ...
      var targetCont = this.roomManager.room.controller.pos.findInRange(FIND_STRUCTURES, 4, {
        filter: (s) => logger.log("-----!!!!!!!!!----", s.id, creep.memory.pickupTargetId) && s.structureType == STRUCTURE_CONTAINER && _.sum(s.store) < s.storeCapacity && s.id != creep.memory.pickupTargetId
      });
      if (targetCont.length > 0) {
        creep.memory.targetContainer = targetCont[0].id;
      } else {
