...
let logger = new logger_imported("role.fArcher");
...
utils.extendFunction(obj, "init", function(name, roomManager, flag) {
...
obj.prototype.runCreep = function(creep) {
  ...
  if (creep.pos.roomName == this.roomManager.roomName) {
    ...
    if (creepsClose && this.roomManager.hostiles.length > 0) {
      ...
      var flags = this.roomManager.room.find(FIND_FLAGS, {
        filter: function(i) {
      ...
      if (flags.length > 0) {
        ...
        var structs = this.roomManager.room.find(FIND_HOSTILE_STRUCTURES, {
          filter: function(s) {
