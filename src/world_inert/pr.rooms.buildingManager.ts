 ...
 * import mod  from "manager.building";
 * mod.thing == 'a thing'; // true
 */

import logger_import from "./screeps.logger";
let logger = new logger_import("pr.rooms.buildingManager");
logger.enabled = true;

import helper from "./util.building";


import processClass from "./INeRT.process";
import threadClass from "./INeRT.thread";

class buildingManager extends processClass {

  initThreads() {
    return [
      this.createThread("initTick", "empire"),
      this.createThread("executeBuildPlan", "work"),
    ];
  }
  ...
  mapFlags() {
    ...
    for (var i in Game.flags) {
      ...
      if (flag.pos.roomName == this.roomManager.roomName) {
        if (flag.color == COLOR_BROWN && flag.secondaryColor == COLOR_WHITE) {
          this.buildLocFlag = flag;
        }
        if (flag.color == COLOR_BROWN && flag.secondaryColor == COLOR_BLUE) {
          this.storageLocFlag = flag;
        }
        if (flag.color == COLOR_BROWN && flag.secondaryColor == COLOR_CYAN) {
          this.labsLocFlag = flag;
        }
        if (flag.color == COLOR_BROWN && flag.secondaryColor == COLOR_PURPLE) {
          this.buildDisplayFlag = flag;
        }
        if (flag.color == COLOR_BROWN && flag.secondaryColor == COLOR_RED) {
  ...
  makeBuildPlan() {
    ...
    logger.log('0--0000-------------------------------')
    //make roads
    var pathSpawnToController = PathFinder.search(baseLoc, {
    ...
    this.buildPlan.extensions = _.uniq(this.buildPlan.extensions, function(loc) {
      return loc.roomName + ":" + loc.x + "," + loc.y
    });
    ...
    //add spawn in base
    this.buildPlan.spawns.push({
      x: baseLoc.x - 1,
      y: baseLoc.y,
      roomName: baseLoc.roomName
    });
    //add spawnContainer
    this.buildPlan.containers.push({
      x: baseLoc.x,
      y: baseLoc.y,
      roomName: baseLoc.roomName
    })
    //add towers in base
    this.buildPlan.towers.push({
      x: baseLoc.x + 1,
      y: baseLoc.y,
      roomName: baseLoc.roomName
    });
    ...
    if (this.storageLocFlag) {
      pathToStorage = PathFinder.search(baseLoc, {
        pos: this.storageLocFlag.pos,
        range: 0
      }, {
        plainCost: 2,
        swampCost: 10,
    ...
    this.buildPlan.storage = (new RoomPosition(storagePos.x, storagePos.y, storagePos.roomName));
    //this.buildPlan.roads.push({x:storagePos.x, y:storagePos.y, roomName:storagePos.roomName})
    var pos = {
      x: storagePos.x + 1,
      y: storagePos.y,
      roomName: storagePos.roomName
    };
    var pos2 = {
      x: storagePos.x + 1,
      y: storagePos.y + 1,
    ...
    for (var i = 0; i < 4; i++) {
      ...
      this.buildPlan.roads.push((new RoomPosition(pos.x, pos.y, pos.roomName)));
      this.buildPlan.roads.push((new RoomPosition(pos2.x, pos2.y, pos2.roomName)));
    ...
    //add labs
    if (this.labsLocFlag) {
      var pathToLabs = this.getPathTo(storagePos, {
        pos: this.labsLocFlag.pos,
        range: 0
      });
    ...
    if (minerals.length) {
      for (var m in minerals) {
        ...
        this.buildPlan.extractors.push({
          x: mineral.pos.x,
          y: mineral.pos.y,
  ...
  addRoadsAndCOntainerToPosition(startPos, pos, skipCont) {
    var pathToPos = this.getPathTo(startPos, {
      pos: pos,
      range: 1
    });
  ...
  getPathTo(startPos, targets) {
    ...
    var path = PathFinder.search(startPos, targets, {
      ...
      maxOps: 10000,
      roomCallback: function(roomName) {
        return _this.planCostMatrix(roomName)
      }
    });
    return path;
  }
  ...
  clkearExtensionsAlongPath(path) {
    for (var p in path) {
      var pos = path[p];
      logger.log("???", pos)
      _.remove(this.buildPlan.extensions, function(e) {
        return e.x == pos.x && e.y == pos.y && e.roomName == pos.roomName
      })
    }
  }

  planCostMatrix(roomName) {
  ...
  castPath(start, differential, skipSteps, minSteps) {
    if (!skipSteps) {
      skipSteps = 0;
    }
    var pos = (new RoomPosition(start.x, start.y, start.roomName));
  ...
  clipPathForExtentions(path, offStart) {
