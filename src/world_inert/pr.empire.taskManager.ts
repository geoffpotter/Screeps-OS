 ...
 * import mod  from "pr.empire.taskManager";
 * mod.thing == 'a thing'; // true
 */


import logger_import from "./screeps.logger";
let logger = new logger_import("lib.taskManager");
...
import taskTypes from "./tasks.all";
...
class taskManager extends processClass {
  ...
  setTask(proc, task) {
    if (!_.some(this.tasks, {
      ...
      if (!_.some(this.tasksByName[task.name], {
          name: task.name,
          procName: task.procName,
          type: task.type,
          pos: task.pos
  ...
  getTaskByNameAndLoc(name, creep, range = 50, useBiggestTask = false, pos = false) {
    ...
    if (tasks) {
      ...
      let ordered = _.sortBy(activeTasksInRange, function(s) {
