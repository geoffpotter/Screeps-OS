...
export class ResourceWrapper extends GameObjectWrapper<Resource> implements StorableCreatableClass<ResourceWrapper, typeof ResourceWrapper, ResourceWrapperData> {
  resourceType: ResourceConstant;
  amount: number;
  nearController: boolean = false;
  private _actionPickup?: BasePickup;
  ...
  getActionPickup(): BasePickup {
    if (!this._actionPickup) {
      this._actionPickup = new PickupAny(this.wpos, this.store);
    }
    return this._actionPickup;
  }
  ...
  delete() {
    ...
    super.delete();
    if (this._actionPickup) {
      this._actionPickup.unassignAll();
    }
  }
